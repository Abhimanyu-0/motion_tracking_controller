...
def setup_controllers(context):
    ...
    use_generalist_value = LaunchConfiguration('use_generalist').perform(context)
    motion_path_value = LaunchConfiguration('motion_path').perform(context)
    ...
    if use_generalist_value.lower() in ["true", "1", "yes"]:
        kv_pairs.append(('walking_controller.motion.use_generalist', 'true'))
    if motion_path_value:
        abs_motion_path = os.path.abspath(os.path.expanduser(os.path.expandvars(motion_path_value)))
...
def generate_launch_description():
    ...
    return LaunchDescription([
        ...
        DeclareLaunchArgument(
            'use_generalist',
            default_value='false',
            description='Use generalist policy mode (true/false)'
        ),
        DeclareLaunchArgument(
            'motion_path',
            default_value='',
            description='Absolute or ~-expanded path to .npz motion file (generalist only)'
        ),
        DeclareLaunchArgument(
            'ext_pos_corr',
            default_value='false',
